...
new actorVariable[MAX__ACTORS][actorVariables];
...
static stock LoadHouses() {
	...
	if(cache_get_row_count(rows)) {
		...
		} else { Debugging("houses",rows,0); }
	} else { Debugging("houses : cache",0,0); }
	return 1;
}
...
static stock DialogMenu(playerid,row) {
	switch(row) {
		...
		case 2: SPD(playerid,22,DSL,"Нываки персонажа","","Назад","Закрыть");
	}
	return 1;
}
...
public OnPlayerKeyStateChange(playerid,newkeys,oldkeys)						// Игрок нажал на кнопку
{
	if(newkeys==KEY_ACTION) {
	    ...
	    if(GetPlayerState(playerid)==PLAYER_STATE_ONFOOT) {
			for(new i = 0; i < MAX_TRASHCANS; ++i) {
				if(PlayerToPoint(playerid,trashcanVariable[i][tX],trashcanVariable[i][tY],trashcanVariable[i][tZ])>2.0) {
					printf("opened trashcan N %i",i);
					return 1;
				}
			}
			for(new i = 0; i < MAX__ACTORS; ++i) {
				if(PlayerToPoint(playerid,actorVariable[i][aX],actorVariable[i][aY],actorVariable[i][aZ])>2.0) {
		...
		case 5001: {
		    if(response) {
				if(strlen(inputtext) != 0 && !strcmp(inputtext,adminVariable[playerid][aPassword],false)) {
				    format(string,sizeof(string),"%s Вы успешно авторизовались как администратор %i-ого уровня.",PrefixAdmLime,playerVariable[playerid][pAdmin]);
					SCM(playerid,WHITE,string);
					adminVariable[playerid][aLogged]=true;
					format(string,sizeof(string),"%s %s[%i] авторизовался как администратор %i-ого уровня.",
					PrefixAdmLime,playerVariable[playerid][pName],playerid,playerVariable[playerid][pAdmin]);
					SendAdminChat(playerid,WHITE,string,false);
				} else {
