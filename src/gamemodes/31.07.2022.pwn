...
static const stock playerSex[][enumIDandName] = {
	{0, "мужской"},
	{1, "женский"}
};

static const stock playerSkinColor[][enumIDandName] = {
	{0, "светлый"},
	{1, "темный"}
};

static const stock playerColorName[][enumIDandName] = {
	{0, COLOR_NONE},
	{1, COLOR_MASK}
};

static const stock playerStatus[][enumIDandName] = {
	{0, "Игрок"},
	{1, "Premium-игрок"},
	{2, "VIP-игрок"}
};
...
static stock const jobNames[][enumJSON] = {
	{0, "Отсутствует"},
	{1, "Такси"},
	{2, "Водитель автобуса"},
	{3, "Механик"},
...
static stock LoadOrganizations() {
	...
	if(cache_get_row_count(rows)) {
		...
		} else { Debugging("organizations",rows,12); }
...
CMD:udost(playerid,params[]) {
	...
	SPD(playerid,11111,DSM,"Удостоверение",string,"Закрыть","");
