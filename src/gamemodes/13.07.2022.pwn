...
static const stock vehicleParam[][vehicleParams] = {
	{400,	"Landstalker",		88,		80},
	{401,	"Bravura",			83,		40},
	{402,	"Buffalo",			104,	50},
	{403,	"Linerunner",		61,		140},
	{404,	"Pereniel",			74,		40},
	{405,	"Sentinel",			91,		50},
	{406,	"Dumper",			61,		200},
	{407,	"Firetruck",		82,		100},
	{408,	"Trashmaster",		66,		80},
	{409,	"Stretch",			88,		60},
	{410,	"Manana",			72,		50},
	{411,	"Infernus",			122,	80},
	{412,	"Voodoo",			95,		60},
	{413,	"Pony",				61,		110,		8},
	{414,	"Mule",				68,		110,		12},
	{415,	"Cheetah",			107,	70,		8},
	{416,	"Ambulance",		88,		80,		10},
	{417,	"Leviathan",		0,		190,		15},
	{418,	"Moonbeam",			68,		70,		10},
	{419,	"Esperanto",		82,		50,		8},
	{420,	"Taxi",				81,		60,		10},
...
new VehTaxi[10];
new VehMechanic[10];*/
new VehTrasher[6][truckVariables];
/*new VehDelivery[10];
new VehTruck[10];
new VehCollector[10];
...
static stock SendAdminChat(playerid,string_color,const string_string[],rank=1,dont_show_to_self=false) {
	foreach(new i:Player) {
...
////////////////////////////////////////////////////////////////////////////////
new filenamePaymentForJob[18] = "PaymentForJob.txt";
static stock PaymentForJob() {
	/*new currYear,currMonth,currDay,currHour,currMinut,currSecond;
	getdate(currYear,currMonth,currDay);
	gettime(currHour,currMinut,currSecond);
	new lastHour[32],nextHour[32];
	format(nextHour,sizeof(nextHour),"%i.%i.%i %i:%i:%i.txt",currDay,currMonth,currYear,currHour,currMinut,currSecond);
	if(fexist())
	if(!fexist(nextHour)) {
		new File:file = fopen(nextHour,io_write);
		fclose(file);
	}*/
	if(fexist(filename)) {
		new File:file = fopen(filenamePaymentForJob,io_read);
		new length = flength(file);
		new data[512],symbol[2];
		...
		new File:file = fopen(filenamePaymentForJob,io_write);
		fclose(file);
	}
}

static stock CheckPaymentForJob() {
	if(!fexist(filename)) {
		new File:file = fopen(filenamePaymentForJob,io_write);
		fclose(file);
	}
...
static stock AddPaymentForJob(playerid,salary) {
    CheckPaymentForJob();
	format(string,sizeof(string),"||");
	new File:file = fopen(filenamePaymentForJob,io_append);
	fwrite(file,string);
	fclose(file);
}
...
CMD:veh(playerid,params[])
{
	...
	if((params[0]==594 || params[0]==592 || params[0]==590 || params[0]==584 || params[0]==577 || params[0]==570 ||
		...
		params[0]==430 || params[0]==425 || params[0]==406) && playerVariable[playerid][pAdmin] < 6)
